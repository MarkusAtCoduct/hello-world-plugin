...
app.post("/connect-tenant", async (req, res) => {
  ...
  const sessionTokenPayload = await verifySessionTokenAndExtractPayload(
    sessionToken
  );
...
async function verifySessionTokenAndExtractPayload(
  sessionToken: string
...
app.all(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const erpInfoResponse = await fetch(
        `https://api${Environment}.jtl-cloud.com/erp/${endpoint}`,
        options
      );
